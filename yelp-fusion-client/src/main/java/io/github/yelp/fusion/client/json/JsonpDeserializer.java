...
import io.github.yelp.fusion.client.util.TriFunction;
...
public interface JsonpDeserializer<V> {
    ...
    Logger logger = LoggerFactory.getLogger(JsonpDeserializer.class);
    ...
    static <T>JsonpDeserializer<T> of(Type type) {
        return new JsonpDeserializerBase<T>(EnumSet.allOf(Event.class)) {
            @Override
