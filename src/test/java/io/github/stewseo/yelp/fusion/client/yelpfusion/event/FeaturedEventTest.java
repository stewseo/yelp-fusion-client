...
public class FeaturedEventTest {
    ...
    public void featuredEventTest() throws Exception {
        ...
        assertThat(response.toString().length()).isGreaterThanOrEqualTo(10);

        assertThat(response.event().id()).isNotNull();
        assertThat(response.event().category()).isNotNull();
        assertThat(response.event().event_site_url()).isNotNull();
        assertThat(response.event().cost_max()).isNotNull();
        assertThat(response.event().toString().getBytes()).contains(new byte[]{123, 34, 99, 97, 116, 101, 103, 111, 114, 121, 34});
