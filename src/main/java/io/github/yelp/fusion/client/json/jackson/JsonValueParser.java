        ...
        switch (parser.currentToken()) {
            ...
            case VALUE_NUMBER_INT:
                return switch (parser.getNumberType()) {
                    case INT -> provider.createValue(parser.getIntValue());
                    case LONG -> provider.createValue(parser.getLongValue());
                    case FLOAT, DOUBLE ->
                        // Use double also for floats, as JSON-P has no support for float
                            new DoubleNumber(parser.getDoubleValue());
                    case BIG_DECIMAL -> provider.createValue(parser.getDecimalValue());
                    case BIG_INTEGER -> provider.createValue(parser.getBigIntegerValue());
                };
